      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        {VERTICALS.map((v, i) => (
          <Reveal key={v.title} delay={i * 60}>
            {base ? (
              <Link to={`${base}/verticals/${v.slug}`} className="group block h-full rounded-2xl p-6 border border-[color:var(--c-border)] bg-[color:var(--c-surface)] hover:-translate-y-1 transition-all duration-300 hover:shadow-lg hover:border-[color:var(--c-accent)]/50">
...
export function NewsCard({ n, base }: { n: NewsArticle; base: string }) {
  return (
    <Link to={`${base}/news/${n.slug}`} className="group block rounded-2xl overflow-hidden border border-[color:var(--c-border)] bg-[color:var(--c-surface)] hover:-translate-y-1 transition-transform duration-300">
...
export function NewsIndexSection({ base }: { base: string }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16 space-y-12">
      {/* Featured */}
      <Reveal>
        <Link to={`${base}/news/${featured.slug}`} className="group grid md:grid-cols-2 gap-6 rounded-3xl overflow-hidden border border-[color:var(--c-border)] bg-[color:var(--c-surface)]">
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 pb-16 grid md:grid-cols-2 gap-4">
        {prev ? (
          <Link to={`${base}/news/${prev.slug}`} className="rounded-2xl p-5 border border-[color:var(--c-border)] bg-[color:var(--c-surface)] hover:-translate-y-1 transition-transform">
        ...
        {next ? (
          <Link to={`${base}/news/${next.slug}`} className="rounded-2xl p-5 border border-[color:var(--c-border)] bg-[color:var(--c-surface)] hover:-translate-y-1 transition-transform text-right">
...
export function EventCard({ e, base }: { e: EventItem; base: string }) {
  ...
  return (
    <Link to={`${base}/events/${e.slug}`} className="group block rounded-2xl overflow-hidden border border-[color:var(--c-border)] bg-[color:var(--c-surface)] hover:-translate-y-1 transition-transform duration-300">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-stretch rounded-3xl overflow-hidden border border-[color:var(--c-border)] bg-[color:var(--c-bg)]">
            ...
            <div className="p-8 md:p-10 flex flex-col justify-center">
              ...
              <div className="mt-6 flex gap-3">
                <Link to={`${base}/events/${upcoming.slug}`} className="inline-flex items-center gap-2 rounded-full bg-[color:var(--c-accent)] text-white px-5 py-2.5 text-sm font-semibold">Event Details <ArrowRight className="h-4 w-4" /></Link>
